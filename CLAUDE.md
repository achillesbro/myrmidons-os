...
carries `chain_id` (missing = 999, pre-v5), the page header has a chain
toggle (HYPEREVM / ROBINHOOD / ALL, state in `app/tools/mnemon/page.tsx`,
filters both tabs), and the ALL view tags each market row with its chain
(`chainTag` in `lib/mnemon/format.ts` — also home of `MNEMON_CHAINS`/`chainOf`).
