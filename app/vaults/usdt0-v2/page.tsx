...
function Usdt0V2VaultPageContent() {
  ...
  // Fetch vault data
  const metadataQuery = useVaultMetadata(HEGEMON_V2_VAULT_ADDRESS, HEGEMON_V2_VAULT_CHAIN_ID, true);
  const apyQuery = useVaultApy(HEGEMON_V2_VAULT_ADDRESS, HEGEMON_V2_VAULT_CHAIN_ID, true);
  ...
  const marketsQuery = useVaultMarkets(HEGEMON_V2_VAULT_ADDRESS, HEGEMON_V2_VAULT_CHAIN_ID, true);
    ...
    async function fetchUserBalance() {
      try {
        ...
        const [decimals, balance] = await Promise.all([
          readVaultDecimals(HEGEMON_V2_VAULT_ADDRESS, client),
          client.readContract({
...
function Usdt0V2VaultPageFallback() {
  return (
...
export default function Usdt0V2VaultPage() {
  return (
    <Suspense fallback={<Usdt0V2VaultPageFallback />}>
      <Usdt0V2VaultPageContent />
    </Suspense>
  );
}
