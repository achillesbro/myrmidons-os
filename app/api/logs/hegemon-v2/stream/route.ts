// HEGEMON_V2 keeper log stream: same proxy as /api/logs/stream, pointed
// at the V2 log-streamer (logs.myrmidons-strategies.com/v2/sse).
...
export async function GET(request: NextRequest) {
  const logStreamUrl = process.env.LOG_STREAM_URL_V2;
  const logStreamToken = process.env.LOG_STREAM_TOKEN_V2;
    ...
    return Response.json(
      {
        error: {
          code: "MISSING_CONFIG",
          message: "LOG_STREAM_URL_V2 and LOG_STREAM_TOKEN_V2 must be configured",
        },
      },
      { status: 500 }
    );
