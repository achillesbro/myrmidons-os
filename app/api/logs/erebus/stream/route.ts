...
export async function GET(request: NextRequest) {
  const logStreamUrl = process.env.EREBUS_LOG_STREAM_URL || "https://logs.myrmidons-strategies.com/erebus/sse";
  const logStreamToken = process.env.EREBUS_LOG_STREAM_TOKEN;
  ...
  try {
    ...
    if (logStreamToken) {
      headers.Authorization = `Bearer ${logStreamToken}`;
    }
