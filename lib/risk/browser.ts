...
export async function fetchMetricHistory(
  ...
  const res = await fetch(`/api/risk/markets/${marketId}/history/${metric}`);
  if (!res.ok) throw new Error(`risk history failed: ${res.status}`);
  return MetricHistorySchema.parse(await res.json());
}
