      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ...
            ["vendor_evidence", "registry · canonical-contract · code-signature · description", "How a leg's publisher was established. registry = the publisher's own feed list; canonical-contract = the adapter points at the publisher's official contract; description = the feed only says so itself, vendor stays null"],
            ["source_name · upstream", "verified contract name · [{getter, address}]", "The oracle's verified source name (proxies resolved), and the feeds an adapter reads through its ABI — an Oval wrapper points at the Chainlink proxy it re-serves"],
          ],
        },
        {
          kind: "p",
          text: "A vendor is asserted only from evidence a third party cannot imitate by copying an interface: the publisher's own registry (Chainlink's feed directory, RedStone's relayer manifests) or the adapter pointing at the publisher's canonical contract (Pyth, Stork, API3). A feed that merely describes itself as a brand is served with a null vendor; the site then names the claimed brand with a '?' mark. Custom oracles get their verified contract name from Sourcify, which drives the family (Curve LP, Pendle PT, Oval wrapper, ...). Oracles that compose a derived leg — an ERC4626 exchange rate, a Curve LP, a Pendle PT, a hardcoded peg — deviate from spot by construction. The site marks their deviation STRUCT and does not flag it as a depeg.",
        },
      ],
    },
    {
